...
def plot_results(results, n_values, p_values, s_values):
    ...
    for i, s in enumerate(s_values):
        plt.plot(s_values, correlations, label=f'{s}', marker='o')

    plt.title('Korrelation Walk zu Degree für variables s von n*10 bis n*n')
    ...
    plt.grid()
    plt.show()
    plt.savefig("KorrelationWalkZuDegree_variable_s.png")
    pass
...
s_values = np.linspace(10000, 1000000, 10,endpoint=True).tolist()
